...
func TestLevel_PlayerTopUpTreasures(t *testing.T) {
	source := rand.New(rand.NewSource(randomSeedTest))
	mapSize := primitives.Size2D[uint]{Height: 30, Width: 90}
	...
	tests := []struct {
		...
		{
			name:      "TotalAmountTreasureOne",
			wantValue: 80,
		},
		{
			name:      "TotalAmountTreasureTwo",
			wantValue: 87,
		},
		{
			name:      "TotalAmountTreasureTree",
			wantValue: 106,
		},
		{
			name:      "TotalAmountTreasureFour",
			wantValue: 123,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			level.PlayerTopUpTreasures()

			if level.Player.Treasures != tt.wantValue {
				t.Errorf("want treasures '%d', but got '%d'", tt.wantValue, level.Player.Treasures)
			}
		})
	}
}
